...
export default function Home() {
  ...
  return (
    <div className="w-full mt-40 px-0 sm:px-3 md:px-6 flex justify-center mb-20">
  ...
  function QuestionsSelection() {
    return (
      <Section className="w-full max-w-4xl bg-foreground rounded-[30px] items-center py-5">
        <HStack className="items-center" spacing={8}>
          <motion.p
            className="
                md:text-3xl
                sm:text-2xl
  ...
  function setQuestionCount(nextCount: number) {
    ...
    setQuestions(prev => {
      ...
      if (prev.length < nextCount) {
        const toAdd = Array.from({ length: nextCount - prev.length }, () => makeQuestion())
        return [...prev, ...toAdd]
      }

      // prev.length > nextCount
  ...
  return (
    <Section className="w-full max-w-4xl rounded-[30px] items-center">
      <VStack className="items-center w-full" spacing={16}>
        {questions.map((q, qi) => (
          <div key={q.id} className="w-full rounded-[30px] bg-foreground p-5">
            <Text className="text-accent font-semibold mb-2">
              Question {qi + 1}
            </Text>

            <input
              className="w-full rounded-2xl bg-sub1 text-sub2 px-4 py-3 outline-none"
              ...
              {q.answers.map((a, ai) => (
                <div key={a.id} className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    title={a.isCorrect ? "Marked correct" : "Marked incorrect"}
                  >
